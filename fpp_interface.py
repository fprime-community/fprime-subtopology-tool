...
def calculateDependencies(input_file):
    ...
    try:
        fprimeUtil = subprocess.run(["fprime-util", "info"], check=True, stdout=subprocess.PIPE)
        ...
        if not buildCache:
            ...
            print("[ERR] fprime-util info did not return an absolute path for build cache")
            sys.exit(1)
    ...
    dependencies = None
    try:
        fppDep = subprocess.run(["fpp-depend", buildCache + "/locs.fpp", input_file], check=True, stdout=subprocess.PIPE)
...
def fpp_to_json(input_file):
    ...
    try:
        fppToJSON = subprocess.run(["fpp-to-json", input_file, "-s"], check=True, stdout=subprocess.PIPE)
...
def fpp_format(input_file):
    ...
    try:
        fppFormat = subprocess.run(["fpp-format", input_file], check=True, stdout=subprocess.PIPE)
