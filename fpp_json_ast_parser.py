class ModuleParser:
    module_JSON = {}
    module_name = None
    module_preannot = None
    module_postannot = None
    ...
    def parse(self):
        self.module_name = self.module_JSON["DefModule"]["node"]["AstNode"]["data"]["name"]
...
class ConstantParser:
    ...
    constant_Id = None
    constant_value = None
    constant_preannot = None
    constant_postannot = None
    ...
    def parse(self):
        self.constant_Id = self.constant_JSON["DefConstant"]["node"]["AstNode"]["data"]["name"]
        
        constant_Value_JSON = self.constant_JSON["DefConstant"]["node"]["AstNode"]["data"]["value"]
...
class InstanceSpecParser:
    ...
    instance_preannot = None
    instance_postannot = None
    ...
    def parse(self):
        instance_bit_1 = self.instance_JSON["SpecCompInstance"]["node"]["AstNode"]["data"]["instance"]
            
        self.instance_name = value_parser(instance_bit_1)
        
        if "Public" in self.instance_JSON["SpecCompInstance"]["node"]["AstNode"]["data"]["visibility"]:
            self.instance_visibility = ""
        elif "Private" in self.instance_JSON["SpecCompInstance"]["node"]["AstNode"]["data"]["visibility"]:
...
class InstanceParser:
    ...
    def parse_phases(self):
        initSpecs = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["initSpecs"]
        
        for initSpec in initSpecs:
            if "phase" in initSpec[1]['AstNode']['data']:
                specType = initSpec[1]['AstNode']['data']['phase']['AstNode']['data']['ExprDot']['id']['AstNode']['data']
                specCode = initSpec[1]['AstNode']['data']['code']
                
                try:
                    self.instance_elements["phases"][specType] = specCode
                except:
                    print("[WARN] Phase type not found in instance JSON [InstanceParser.parse_phases]")
                    pass
                        
    def parse(self):
        self.instance_name = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["name"]
        
        components_bit_1 = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["component"]
            
        self.instance_elements["component_name"] = value_parser(components_bit_1)
        self.instance_elements["base_id"] = value_parser(self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["baseId"])
                            
        if "queueSize" in self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]:
            self.instance_elements["queue"] = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["queueSize"]
        if "stackSize" in self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]:
            self.instance_elements["stack"] = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["stackSize"]
        if "priority" in self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]:
            self.instance_elements["priority"] = self.instance_JSON["DefComponentInstance"]["node"]["AstNode"]["data"]["priority"]
            
        if self.instance_elements["queue"] is not None and self.instance_elements["queue"] != "None":
            queueSize_JSON = self.instance_elements["queue"]['Some']['AstNode']['data']
            
            if "ExprLiteralInt" in queueSize_JSON:
                self.instance_elements["queue"] = queueSize_JSON["ExprLiteralInt"]["value"]
                ...
                self.instance_elements["queue"] = qualifier_calculator(queueSize_JSON["ExprDot"])
                                    
        if self.instance_elements["stack"] is not None and self.instance_elements["stack"] != "None":
            stackSize_JSON = self.instance_elements["stack"]['Some']['AstNode']['data']
            
            if "ExprLiteralInt" in stackSize_JSON:
                self.instance_elements["stack"] = stackSize_JSON["ExprLiteralInt"]["value"]
                ...
                self.instance_elements["stack"] = qualifier_calculator(stackSize_JSON["ExprDot"])
                    
        if self.instance_elements["priority"] is not None and self.instance_elements["priority"] != "None":
            priority_JSON = self.instance_elements["priority"]['Some']['AstNode']['data']
            
            if "ExprLiteralInt" in priority_JSON:
                self.instance_elements["priority"] = priority_JSON["ExprLiteralInt"]["value"]
                ...
                self.instance_elements["priority"] = qualifier_calculator(priority_JSON["ExprDot"])
...
class TopologyImport:
    import_JSON = {}
    import_name = None
    import_preannot = None
    import_postannot = None
    ...
    def parse(self):
        import_bit_1 = self.import_JSON["SpecTopImport"]["node"]["AstNode"]["data"]["top"]
...
class ConnectionGraphParser:
    ...
    def parse(self):
        import_bit = self.cg_JSON["SpecConnectionGraph"]["node"]["AstNode"]["data"]["Direct"]
        ...
        for connection in import_bit["connections"]:
            ...
            connectionToAppend["source"]["name"] = value_parser(compInst) + "." + fromPort['portName']['AstNode']['data']
            connectionToAppend['source']['num'] = ("[" + connection['fromIndex'] + "]") if connection['fromIndex'] != "None" else ""
                        
            toPort = connection["toPort"]['AstNode']['data']
            compInst = toPort['componentInstance']
            
            connectionToAppend["dest"]["name"] = value_parser(compInst) + "." + toPort['portName']['AstNode']['data']
            connectionToAppend['dest']['num'] = ("[" + connection['toIndex'] + "]") if connection['toIndex'] != "None" else ""
...
class TopologyParser:
    topology_JSON = {}
    topology_name = None
    topology_preannot = None
    topology_postannot = None
    ...
    def parse(self):
        self.topology_name = self.topology_JSON["DefTopology"]["node"]["AstNode"]["data"]["name"]
...
def qualifier_calculator(qualifier_JSON):
    ...
    if "ExprDot" in next_idx:
        path += qualifier_calculator(next_idx['ExprDot']) + "." + next_idx['ExprDot']['id']['AstNode']['data']
...
def value_parser(value_JSON):
    ...
    if "Unqualified" in checker:
        ...
        qf = checker["Qualified"]['qualifier']['AstNode']['data']['Unqualified']['name'] + "." + checker["Qualified"]['name']['AstNode']['data']
        return qf
...
def parse_binop(constant_JSON):
    ...
    return binop + " " + Binops(list(constant_JSON["op"].keys())[0]) + " " + value_parser(constant_JSON[f"e{idx_RHS}"])
...
def parse_array(constant_JSON): 
    ...
    for i in range(len(constant_Value_JSON)):    
        if "ExprArray" not in constant_Value_JSON[i]['AstNode']['data']:
            ...
            arrayOpen = arrayOpen + parse_array(constant_Value_JSON[i]['AstNode']['data']) + ", "
...
def parse_struct(constant_JSON):
    ...
    for param in constant_JSON["ExprStruct"]["members"]:
        structOpen = structOpen + "    " + param['AstNode']['data']['name'] + " = " + value_parser(param['AstNode']['data']['value']) + ",\n"
